...
import { getSubscriberAudioById, updateSubscriberAudioById } from 'apiSdk/subscriber-audios';
...
function SubscriberAudioEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<SubscriberAudioInterface>(
    () => (id ? `/subscriber-audios/${id}` : null),
    () => getSubscriberAudioById(id),
  );
  ...
  const handleSubmit = async (values: SubscriberAudioInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSubscriberAudioById(id, values);
      mutate(updated);
      resetForm();
