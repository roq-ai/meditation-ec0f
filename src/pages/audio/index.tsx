...
type ColumnType = ColumnDef<AudioInterface, unknown>;
...
export function AudioListPage(props: AudioListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getAudio({
        relations: ['organization', 'series_audio.count', 'subscriber_audio.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['title.contains', 'category.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AudioInterface>>(
    () => `/audio?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('series_audio', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('subscriber_audio', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'subscriber_audio',
          header: 'Subscriber Audio',
          accessorKey: 'subscriber_audio',
          cell: ({ row: { original: record } }: any) => record?._count?.subscriber_audio,
        }
      : null,

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/audio/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
